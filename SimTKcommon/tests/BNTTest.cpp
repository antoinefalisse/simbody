...
int main() {
  ...
  { const Real zero = 0., one = 1., two = 2.;
    ...
    const SimTK::Real dnan = NTraits<SimTK::Real>::getNaN();
...
#define STRZ(X) STRZ_(X)
    ...
    //     << " d=" << setprecision(NTraits<SimTK::Real>::getNumDigits()+2) << NTraits<SimTK::Real>::getPi()
    ...
    //     << " d=" << setprecision(NTraits<SimTK::Real>::getNumDigits()+2) << NTraits<SimTK::Real>::getOneOverSqrt2()
