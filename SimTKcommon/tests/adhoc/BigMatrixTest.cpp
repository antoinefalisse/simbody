...
int main()
{
    try {
        ...
        //Vector_<SimTK::Real> mmRowScaleR(3); for (int i = 0; i<3; ++i) mmRowScaleR[i] = (float)mmRowScale[i];
        ...
        //mm2(0, 2, 2, 2) = std::complex<SimTK::Real>(-99, -99);
        ...
        Matrix_<Mat<2, 2, Mat<2, 2, SimTK::Real> > > mmm(2, 1);
        mmm = Mat<2, 2, Mat<2, 2, SimTK::Real> >(Mat<2, 2, SimTK::Real>(1));
        cout << "*****>>> mmm=" << mmm << endl;
       ...
       /* const Real rdata[] = { 1,2,3,
        ...
        try {
            const SimTK::Real ddd[] = { 11, 12, 13, 14, 15, 16 };
        ...
        //for (int i = 0; i<LUP; ++i)
        //    flip = big.invert();
        //cout << "... DONE -- " << (SimTK::Real)(std::clock() - start) / CLOCKS_PER_SEC << " seconds" << endl << std::flush;
        ...
        //cout << "... DONE -- " << (SimTK::Real)(std::clock() - start) / CLOCKS_PER_SEC << " seconds" << endl << std::flush;
        //cout << "RMS (big*flip).norm() - 1=" << sqrt(ans.normSqr() / N) - 1. << endl << std::flush;
        ...
        //cout << "... DONE -- " << (SimTK::Real)(std::clock() - start) / CLOCKS_PER_SEC << " seconds" << endl << std::flush;
        //cout << "RMS (big*nrflip).norm() - 1=" << sqrt(ans.normSqr() / N) - 1. << endl << std::flush;
