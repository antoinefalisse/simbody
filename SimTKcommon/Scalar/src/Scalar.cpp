...
template class CNT< negator< conjugate<long double> > >;
    ...
    template bool isNumericallyEqual(const T&, const Recorder&,                  double tol); \
...
// Don't duplicate anything instantiated with the previous macro.
    ...
    template bool isNumericallyEqual(const Recorder&,                const T&, double tol); \
...
#ifdef SimTK_REAL_IS_ADOUBLE
    template class negator<Recorder>;
    template class CNT< negator<Recorder> >;
#endif
