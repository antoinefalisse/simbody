...
/*  Initialization of constants for shift rotation. */        
    ...
    rot = (T) 94.0;
    rot *= (T) 0.017453293;
    cosr = NTraits<T>::cos(rot);
	sinr = NTraits<T>::sin(rot);
    n = degree;
 ...
 */
    ...
    l = (int)(NTraits<T>::log(sc)/ NTraits<T>::log(base) + 0.5);
...
/*  Compute upper estimate of bound. */
    x = NTraits<T>::exp((NTraits<T>::log(-pt[n])- NTraits<T>::log(pt[0])) / (T)n);
...
// Fixed version:
    if ((T)fabs(fabs(szr)-fabs(lzr)) > (T)0.01 * std::max((T)std::abs(lzr),(T)0.1)) 
        return;
...
int RPoly<Recorder>::findRoots(Recorder *op, int degree, Recorder *zeror, Recorder *zeroi)
{
	Recorder t, aa, bb, cc, *temp, factor, rot;
	Recorder *pt;
	Recorder lo, max, min, xx, yy, cosr, sinr, xxx, x, sc, bnd;
	Recorder xm, ff, df, dx, infin, smalno, base;
	int cnt, nz, i, j, jj, l, nm1, zerok;
	/*  The following statements set machine constants. */
	base = 2.0;
	/*eta = std::numeric_limits<Recorder>::epsilon();*/
	eta = SimTK::NTraits<Recorder>::getEps();
	//infin = Recorder(std::numeric_limits<Recorder>::max());
	infin = SimTK::NTraits<Recorder>::getInfinity();
	//smalno = Recorder(std::numeric_limits<Recorder>::min());
	smalno = SimTK::NTraits<Recorder>::getLeastNegative();
	are = eta;
	mre = eta;
	lo = smalno / eta;
	/*  Initialization of constants for shift rotation. */
	...
	xx = sqrt(Recorder(0.5));
	yy = -xx;
	rot = (Recorder) 94.0;
	rot *= (Recorder) 0.017453293;
	cosr = cos(rot);
	sinr = sin(rot);
	n = degree;

	/*  Algorithm fails if the leading coefficient is zero (or if degree==0). */
	if (n < 1 || op[0] == Recorder(0.0))
		return -1;
	/*  Remove the zeros at the origin, if any. */
	while (op[n].getValue() == 0.0) {
		j = degree - n;
		zeror[j] = 0.0;
		zeroi[j] = 0.0;
		n--;
	...
	*  Allocate memory here
	*/
	temp = new Recorder[degree + 1];
	pt = new Recorder[degree + 1];
	p = new Recorder[degree + 1];
	qp = new Recorder[degree + 1];
	k = new Recorder[degree + 1];
	qk = new Recorder[degree + 1];
	svk = new Recorder[degree + 1];
	/*  Make a copy of the coefficients. */
	for (i = 0; i <= n; i++)
		p[i] = op[i];
...
_40:
	...
	for (i = 0; i <= n; i++) {
		...
		if ((x.value() != 0.0) && (x.value() < min.value())) {
			min = x;
		}
	}
	/*  Scale if there are large or very small coefficients.
	...
	*/
	sc = lo / min;
	if (sc.value() > 1.0 && infin.value() / sc.value() < max.value()) goto _110;
	if (sc.value() <= 1.0) {
		if (max.value() < 10.0) goto _110;
		if (sc.value() == 0.0)
			sc = smalno;
	}
	l= (log(sc) / log(base) + Recorder(0.5)).value();
	factor = pow(base*(Recorder)1.0, l);
	if (factor.value() != 1.0) {
		for (i = 0; i <= n; i++)
			p[i] = factor*p[i];     /* Scale polynomial. */
	}
_110:
	...
	pt[n] = -pt[n];
	/*  Compute upper estimate of bound. */
	x = exp((log(-pt[n]) - log(pt[0])) / (Recorder)n);
	/*  If Newton step at the origin is better, use it. */
	if (pt[n - 1] != 0.0) {
		xm = -pt[n] / pt[n - 1];
		if (xm < x)  x = xm;
	...
	*/
	nm1 = n - 1;
	for (i = 1; i<n; i++)
		k[i] = (Recorder)(n - i)*p[i] / (Recorder)n;
	k[0] = p[0];
	aa = p[n];
	bb = p[n - 1];
	zerok = (k[n - 1] == 0);
...
*/
template <class Recorder>
void RPoly<Recorder>::fxshfr(int l2, int *nz)
{
	Recorder svu, svv, ui, vi, s;
	Recorder betas, betav, oss, ovv, ss, vv, ts, tv;
	Recorder ots, otv, tvv, tss;
	int type, i, j, iflag, vpass, spass, vtry, stry;
	...
	*nz = 0;
	...
	for (j = 0; j<l2; j++) {
		...
		/*  Estimate s. */
		ss = 0.0;
		if (k[n - 1].value() != 0.0) ss = -p[n] / k[n - 1];
		tv = 1.0;
		ts = 1.0;
		if (j == 0 || type == 3) goto _70;

		/*  Compute relative measures of convergence of s and v sequences. */
		if (vv.getValue() != 0.0) tv = fabs((vv - ovv) / vv);
		if (ss.getValue() != 0.0) ts = fabs((ss - oss) / ss);
		/*  If decreasing, multiply two most recent convergence measures. */
		tvv = 1.0;
		if (tv < otv) tvv = tv*otv;
		tss = 1.0;
...
*/
template <class Recorder>
void RPoly<Recorder>::quadit(Recorder *uu, Recorder *vv, int *nz)
{
	Recorder ui, vi;
	Recorder mp, omp, ee, relstp, t, zm;
	int type, i, j, tried;
	...
	*nz = 0;
	...
	/*  Main loop. */
_10:
	quad(Recorder(1.0), u, v, &szr, &szi, &lzr, &lzi);
	/*  Return if roots of the quadratic are real and not
	*  close to multiple or nearly equal and of opposite
	*  sign.
	*/
	...
	//if (fabs(fabs(szr)-fabs(lzr)) > 0.01 * fabs(lzr)) return;
	// Fixed version:
	if ((Recorder)fabs(fabs(szr) - fabs(lzr)) > (Recorder)0.01 * fmax((Recorder)fabs(lzr), (Recorder)0.1))
		return;
	...
	*/
	zm = sqrt(fabs(v));
	ee = (Recorder) 2.0*fabs(qp[0]);
	t = -szr*b;
	for (i = 1; i<n; i++) {
		ee = ee*zm + fabs(qp[i]);
	}
	ee = ee*zm + fabs(a + t);
	ee *= ((Recorder)5.0 *mre + (Recorder)4.0*are);
	ee = ee - ((Recorder)5.0*mre + (Recorder)2.0*are)*(fabs(a + t) + fabs(b)*zm) + (Recorder)2.0*are*fabs(t);
	/*  Iteration has converged sufficiently if the
	*  polynomial value is less than 20 times this bound.
	*/
	if (mp <= 20.0*ee) {
...
*/
template <class Recorder>
void RPoly<Recorder>::realit(Recorder *sss, int *nz, int *iflag)
{
	Recorder pv, kv, t, s;
	Recorder ms, mp, omp, ee;
	int i, j;
...
*/
template <class Recorder>
void RPoly<Recorder>::calcsc(int *type)
{
	/*  Synthetic division of k by the quadratic 1,u,v */
	quadsd(n - 1, &u, &v, k, qk, &c, &d);
	if (fabs(c) > fabs(k[n - 1] * 100.0*eta)) goto _10;
...
*/
template <class Recorder>
void RPoly<Recorder>::nextk(int *type)
{
	Recorder temp;
	int i;
	if (*type == 3) {
...
*/
template <class Recorder>
void RPoly<Recorder>::newest(int type, Recorder *uu, Recorder *vv)
{
	Recorder a4, a5, b1, b2, c1, c2, c3, c4, temp;
...
*/
template <class Recorder>
void RPoly<Recorder>::quadsd(int nn, Recorder *u, Recorder *v, Recorder *p, Recorder *q,
...
*/
template <class Recorder>
void RPoly<Recorder>::quad(Recorder a, Recorder b1, Recorder c, Recorder *sr, Recorder *si,
	Recorder *lr, Recorder *li)
{
	Recorder b, d, e;
