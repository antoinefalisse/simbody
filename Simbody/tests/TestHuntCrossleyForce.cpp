...
void testForces() {
	...
	Transform T1;
	T1.setToZero();
    MobilizedBody::Translation sphere(matter.updGround(), T1, body, T1);
    contacts.addBody(setIndex, sphere, ContactGeometry::Sphere(radius), T1);
	Transform T2;
	T2.set(Rotation(-0.5*Pi, ZAxis), Vec3(0));
    //contacts.addBody(setIndex, matter.updGround(), ContactGeometry::HalfSpace(), Transform(Rotation(-0.5*Pi, ZAxis), Vec3(0))); // y < 0
	contacts.addBody(setIndex, matter.updGround(), ContactGeometry::HalfSpace(), T2); // y < 0
    ...
    for (Real height = radius+0.2; height > 0; height -= 0.1) {
		sphere.setQ(state, Vec3(0, height, 0));
        /*sphere.setQToFitTranslation(state, Vec3(0, height, 0));*/
        system.realize(state, Stage::Dynamics);
        const Real depth = radius-height;
        Real f = 0;
        if (depth > 0)
            f = (4.0/3.0)*stiffness*depth*sqrt(radius*stiffness*depth);
    ...
    for (Real height = radius+0.2; height > 0; height -= 0.1) {
        ...
        Real fh = 0;
        if (depth > 0)
            fh = (4.0/3.0)*stiffness*depth*sqrt(radius*stiffness*depth);
        for (Real v = -1.0; v <= 1.0; v += 0.1) {
            sphere.setU(state, Vec3(0, -v, 0));
            system.realize(state, Stage::Dynamics);
    ...
    for (Real height = radius+0.2; height > 0; height -= 0.1) {
        ...
        if (depth > 0)
            fh = (4.0/3.0)*stiffness*depth*sqrt(radius*stiffness*depth);
        for (Real v = -1.0; v <= 1.0; v += 0.1) {
            sphere.setU(state, Vec3(v, 0, 0));
            system.realize(state, Stage::Dynamics);
            const Real vrel = fabs(v/vt);
            Real ff = (v < 0 ? 1 : -1)*fh*(min(vrel, Real(1.0))*(ud+2*(us-ud)/(1+vrel*vrel))+uv*fabs(v));
