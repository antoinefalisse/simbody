...
void SmoothSphereHalfSpaceForceImpl::calcForce(const State& state,
    ...
    const Real k = (1./2.)*NTraits<Real>::pow(stiffness,2./3.);
    const Real fh_pos = (4./3.)*k*NTraits<Real>::sqrt(contactSphereRadius*k)*
        NTraits<Real>::pow(NTraits<Real>::sqrt(
            indentation*indentation+cf),3./2.);
    const Real fh_smooth = fh_pos*(1./2.+(1./2.)*
        NTraits<Real>::tanh(bd*indentation));
    ...
    const Real fhc_smooth = fhc_pos*(1./2.+(1./2.)*NTraits<Real>::tanh(
        bv*(vnormal+(2./(3.*c)))));
    ...
    const Real ff = fhc_smooth*(NTraits<Real>::min(vrel,Real(1))*
